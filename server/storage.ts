...
export class MemStorage implements IStorage {
  ...
  async getBookingsByAgentId(agentId: number): Promise<Booking[]> {
    ...
    return Array.from(this.bookings.values()).filter(
      booking => booking.referralCode === agent.referralCode
    );
  }
  ...
  async createBooking(insertBooking: InsertBooking): Promise<Booking> {
    ...
    const booking: Booking = { 
      ...
      commission: insertBooking.totalAmount * 0.1 // Assuming 10% commission for simplicity
    };
  ...
  async getAgentByReferralCode(referralCode: string): Promise<Agent | undefined> {
    return Array.from(this.agents.values()).find(
      agent => agent.referralCode === referralCode
    );
  }
  ...
  async getAgentStats(agentId: number): Promise<AgentStats> {
    ...
    const totalEarnings = confirmedBookings.reduce((sum, booking) => sum + booking.commission, 0);
